...
class AbstractPPO(metaclass=ABCMeta):
    ...
    def rollout_episodes(self) -> Tuple[float, float]:
        ...
        while number_of_step < self.timestep_per_update:
            ...
            for _ in range(self.timestep_per_episode):
                ...
                mask_env = info.get("mask")

                action = torch.tensor(
                    [action], device=self.device, dtype=torch.float32)
                self.buffer.add_step_to_buffer(
                    reward, value, log_prob, action, done, state, mask,mask_env)
                state = next_state
                number_of_step += 1
    ...
    def get_mask(self, state) -> torch.Tensor:
        ...
        return torch.ones(5, device=self.device)
    ...
    def evaluate(self):
        ...
        for _ in range(10):
            ...
            while not done and iterations <= self.timestep_per_episode:
                action, _ = self.choose_action(state,mask,deterministic=False)
                next_state, reward, done, _, info = self.env.step(action)
                mask = info.get("mask")
                iterations += 1
                if self.render:
